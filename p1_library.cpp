...
// ////////////////////////////// BEGIN csvstream.hpp
// /////////////////////////////////
// //////////// GitHub hash 88b29ce1368421d9bd36eec20712cd25becf0475
// /////////////
// /* -*- mode: c++ -*- */
...
//   // header.
//   csvstream &operator>>(std::vector<std::pair<std::string, std::string>>
//   &row);
...
//   // File stream in CSV format, used when library is called with filename
//   ctor std::ifstream fin;
...
//   // Strictly enforce the number of values in each row.  Raise an exception
//   if
//   // a row contains too many values or too few compared to the header.  When
//   // strict=false, ignore extra values and set missing values to empty
//   string. bool strict;
...
//   }; State state = BEGIN; while (is.get(c)) {
//     switch (state) {
//     case BEGIN:
...
//         data.back() += c;
//       } else if (c == delimiter) {
//         // If you see a delimiter, then start a new field with an empty
//         string data.push_back("");
//       } else if (c == '\n' || c == '\r') {
//         // If you see a line ending *and it's not within a quoted token*,
//         stop
...
//   behavior
//   // of getline(), which will set the eofbit, but *not* the failbit if a
//   partial
//   // line is read.
//   if (state != BEGIN)
//     is.clear();
...
// csvstream::csvstream(const std::string &filename, char delimiter, bool
// strict)
//     : filename(filename), is(fin), delimiter(delimiter), strict(strict),
//       line_no(0) {
...
//     : filename("[no filename]"), is(is), delimiter(delimiter),
//     strict(strict),
//       line_no(0) {
//   read_header();
// }
...
//   // larger than header, discard extra values.  If data is smaller than
//   header,
